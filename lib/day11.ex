defmodule Day11 do
  ...
  def paint_spaceship do
    get_code()
    |> Intcode.new()
    |> run_paint_robot(@black, :up, {0, 0}, %{})
    # get the qty of panels visited at least once
    |> Enum.count()
  end
  ...
  Your puzzle answer was LRZECGFE.
  """

  def registration_id do
    # Makes unit test output prettier
    IO.puts("")

    get_code()
    |> Intcode.new()
    |> run_paint_robot(@white, :up, {0, 0}, %{})
    |> plot_panels()
    |> IO.puts()
  ...
  defp get_code(input \\ "inputs/day11_input.txt") do
    File.read!(input)
    |> String.split(",")
    |> Enum.map(&String.to_integer/1)
  end

  defp run_paint_robot(program, color_in, dir, panel, panels) do
    # Set the panel color input, and run the program twice
    # to get the color painted and the rotation
    program = Intcode.run_input(program, color_in)

    if !Intcode.halted?(program) do
      color_out = Intcode.get_output(program)
      program = Intcode.run(program)
      rot = Intcode.get_output(program)
      # IO.puts("Color: #{color_out}, Rot: #{rot}")

      panels = Map.put(panels, panel, color_out)
      dir = turn(dir, rot)
      panel = move(panel, dir)
      color = Map.get(panels, panel, @black)
      run_paint_robot(program, color, dir, panel, panels)
    else
      # IO.puts("Halted")
      panels
    end
