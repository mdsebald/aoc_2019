defmodule Day07 do
  ...
  def highest_signal_1 do
    get_code()
    |> find_highest_signal_1()
  end

  defp find_highest_signal_1(code) do
    gen_phase_combos([0, 1, 2, 3, 4], 5)
    |> Enum.reduce(0, fn phases, max ->
      max(run_amps(code, phases, 0), max)
    end)
  end

  def test1_1 do
    code = get_code("inputs/day07_test1_1_input.txt")
    phases = [4, 3, 2, 1, 0]
    run_amps(code, phases, 0)
  end

  def test1_2 do
    code = get_code("inputs/day07_test1_2_input.txt")
    phases = [0, 1, 2, 3, 4]
    run_amps(code, phases, 0)
  end

  def test1_3 do
    code = get_code("inputs/day07_test1_3_input.txt")
    phases = [1, 0, 4, 3, 2]
    run_amps(code, phases, 0)
  end

  defp run_amps(_code, [], output), do: output
  ...
  def highest_signal_2 do
    get_code()
    |> find_highest_signal_2()
  end

  defp find_highest_signal_2(code) do
    gen_phase_combos([5, 6, 7, 8, 9], 5)
    |> Enum.reduce(0, fn phases, max ->
      max(init_run_amps(code, phases), max)
    end)
  end

  def test2_1 do
    code = get_code("inputs/day07_test2_1_input.txt")
    phases = [9, 8, 7, 6, 5]
    init_run_amps(code, phases)
  end

  def test2_2 do
    code = get_code("inputs/day07_test2_2_input.txt")
    phases = [9, 7, 8, 5, 6]
    init_run_amps(code, phases)
  end
  ...
  defp get_code(input \\ "inputs/day07_input.txt") do
    File.read!(input)
    |> String.split(",")
    |> Enum.map(&String.to_integer/1)
  end

  defp gen_phase_combos([], _k), do: [[]]

  defp gen_phase_combos(_list, 0), do: [[]]

  defp gen_phase_combos(list, k) do
    for head <- list, tail <- gen_phase_combos(list -- [head], k - 1), do: [head | tail]
  end
end
